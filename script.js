...
// ADDED: Game stats for achievements
let gameStats = {
  totalClicks: 0,
  totalKp: 0,
  upgradesPurchased: 0,
...
const achievementSound = new Audio('sounds/achievement.mp3'); // ADDED: Sound for achievement
clickSound.load();
upgradeSound.load();
prestigeSound.load();
...
function showAchievementToast(name) {
    toastText.textContent = `Achievement Unlocked: ${name}`;
    achievementToast.classList.remove('hidden');
    if (!isMuted) {
        achievementSound.currentTime = 0;
        achievementSound.play().catch(e => console.error("Audio play failed:", e));
    }
    setTimeout(() => {
...
function renderAchievements() {
    ...
    achievements.forEach(ach => {
        ...
        achDiv.innerHTML = `<h4>${ach.name}</h4><p>${ach.description}</p>`;
...
clickBtn.addEventListener("click", () => {
  if (!isMuted) { clickSound.currentTime = 0; clickSound.play().catch(e => console.error("Audio play failed:", e)); }
  const pointsGained = kpPerClick * getPrestigeMultiplier();
  kp += pointsGained;
  gameStats.totalKp += pointsGained; // Track total KP for achievements
  gameStats.totalClicks++; // Track clicks for achievements
  updateDisplay();
});
...
function applyUpgrade(u, cost) {
  ...
  u.effect();
  u.count += 1;
  gameStats.upgradesPurchased++; // Track upgrades for achievements
  updateDisplay();
...
function renderUpgrades() {
  upgrades.forEach((u, idx) => {
    ...
    if (u.unlocked && !btn) {
      ...
      btn.addEventListener("mouseenter", (e) => { tooltip.textContent = u.description; tooltip.classList.remove("hidden"); positionTooltip(e); tooltip.classList.add("visible"); });
      btn.addEventListener("mousemove", positionTooltip);
      btn.addEventListener("mouseleave", () => { tooltip.classList.remove("visible"); setTimeout(() => tooltip.classList.add("hidden"), 200); });
      btn.addEventListener("click", () => {
        const currentCost = Math.floor(u.baseCost * Math.pow(1.15, u.count));
        if (kp >= currentCost) {
          if (!isMuted) { upgradeSound.currentTime = 0; upgradeSound.play().catch(e => console.error("Audio play failed:", e)); }
          applyUpgrade(u, currentCost);
        }
      });
      upgradeContainer.appendChild(btn);
      if (u.unlocked && !u._factShown && facts[u.name]) { factText.textContent = facts[u.name]; factCard.classList.remove("hidden"); u._factShown = true; }
    }
    if (btn) { const cost = Math.floor(u.baseCost * Math.pow(1.15, u.count)); btn.disabled = kp < cost; btn.textContent = `${u.name} (${cost} KP)`; }
  });
}
...
function saveGame() {
  const saveData = {
    kp, kpPerClick, kpPerSecond, prestiges, isMuted,
    upgrades: upgrades.map(u => ({ count: u.count, unlocked: u.unlocked, _factShown: u._factShown })),
    achievements: achievements.map(a => a.unlocked), // Save achievement state
    gameStats, // Save game stats
    glossary: glossaryList.innerHTML // Save glossary content
  };
...
function loadGame() {
  ...
  if (!data) return;
  kp = data.kp || 0; kpPerClick = data.kpPerClick || 1; kpPerSecond = data.kpPerSecond || 0; prestiges = data.prestiges || 0; isMuted = data.isMuted || false;
  gameStats = data.gameStats || { totalClicks: 0, totalKp: 0, upgradesPurchased: 0 };
  if (data.upgrades) { upgrades.forEach((u, i) => { const savedUpgrade = data.upgrades[i]; if (savedUpgrade) { u.count = savedUpgrade.count || 0; u.unlocked = savedUpgrade.unlocked || false; u._factShown = savedUpgrade._factShown || false; } }); }
  if (data.achievements) { achievements.forEach((a, i) => { a.unlocked = data.achievements[i] || false; }); }
  if (data.glossary) { glossaryList.innerHTML = data.glossary; }
  updateMuteButton();
}
...
function gameLoop() {
  ...
  kp += passiveKp;
  gameStats.totalKp += passiveKp; // Track passive KP for achievements
  updateDisplay();
  renderUpgrades();
  checkAchievements(); // Check for new achievements every tick
}
...
window.addEventListener('beforeunload', saveGame); // Save on page unload